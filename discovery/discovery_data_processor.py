...
https://github.com/vlfom/RNCDL/blob/main/discovery/data/discovery_data_processor.py
"""
    ...
    def strong_augmentations_list_swav_no_scalejitter():
        return [
            ...
            RandomImageTransform(
                T.ToPILImage(), prob=1.0
            ),  # Convert to PIL to support applying torchvision's transforms
        ...
        for i in range(self.num_augmentations):
            ...
            # We are in instances transformation mode
                ...
                for p in dataset_dict["instances"]:
                    ...
                    segms.append(mask)
                segms = BitMasks(torch.stack([torch.from_numpy(np.ascontiguousarray(x)) for x in segms]))
